<fieldset class="fieldset" id="tags-input">
    ...
    <input type="text" name="query" value="{{ $query ?? '' }}" class="input w-full"
        ...
        hx-target="#tag-suggestions" hx-select="#tag-suggestions" hx-swap="outerHTML"
        hx-post="{{ route('tags.suggest') }}" />
        ...
        <ul class="flex flex-row flex-wrap gap-2 p-0">
            @if(isset($tags))
                @foreach (explode(",", $tags ?? "") as $tag)
                    ...
                    ]) }}" hx-target="#tag-suggestions" hx-select="#tag-suggestions">
            ...
            @if (isset($suggestions))
                @foreach ($suggestions as $suggestion)
                        ...
                        <script>me().on('click', _ => me('[name="query"]').value = "");</script>
                @endforeach
            @endif
            @if(isset($query))
                        <button class="badge badge-outline cursor-pointer" type="button" hx-post="{{ route('tags.suggest', [
